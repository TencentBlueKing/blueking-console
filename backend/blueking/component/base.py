# -*- coding: utf-8 -*-
"""
TencentBlueKing is pleased to support the open source community by making 蓝鲸智云(BlueKing) available.
...
class ComponentAPI(object):
    ...
    def _call(self, *args, **kwargs):
        ...
        if self.method == "POST":
            ...
            try:
                ...
                raise ComponentAPIException(
                    self, "Request parameter error (please pass in a dictionary or json string)"
                )
        ...
        if resp.status_code != self.HTTP_STATUS_OK:
            message = u"Request error, error status code: %s" % resp.status_code
        ...
        try:
            ...
            if not json_resp["result"]:
                ...
                log_message = (
                    u"The component returned an error message: %(message)s, request_id=%(request_id)s "
            ...
            raise ComponentAPIException(self, u"Return data format is incorrect, unified as json.", resp=resp)
