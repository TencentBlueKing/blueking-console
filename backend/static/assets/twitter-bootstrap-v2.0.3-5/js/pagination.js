...
jQuery.fn.pagination = function(opt){
	...
	var total_num						//异步请求数据总的记录数
	    ...
	    ,panel = $(this)               //分页条对象
		,loading_img = "http://o.qcloud.com/console/static/twitter-bootstrap-v2.0.3-5/img/loading.gif"    //加载图片
	...
	return this.each(function(){
		...
		if(opts.async){
			...
			//根据url请求数据源
			$.get(opts.url+(opts.url.indexOf('?')<0?"?page=":"page=")+opts.current_page,function(data){
				//请求的字符串转换为json对象
