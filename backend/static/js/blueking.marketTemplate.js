...
var apptrTemp = template.compile(
    '<li>'+
        ...
        '</a>'+
        '<a href="javascript:BLUEKING.marketApp.openDetailIframe(<%=relapp_id%>);">'+
            '<span class="app-name"><%=name%><em>(<span style="font-family: 微软雅黑;width:260px;color:#73AEF1;">' + gettext('本月访问量：') + '</span><span style="color:#DFA53A"><%=app_visit_count%></span>)</em></span>'+
        '</a>'+
        '<span class="app-desc" style="width:200px"><%=introduction%></span>'+
        ...
        '</span>'+
        '<span class="app-stat" style="right:80px;top:38px;width:250px;">' + gettext('<%=use_count%> 人添加') + '</span>'+
        '<% if(is_has){ %>'+
